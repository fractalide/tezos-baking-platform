...
{ doCheck ? false, stdenv, opam, fetchurl, ocaml, jbuilder, conduit-lwt-unix,
  cmdliner, magic-mime, logs, fmt, cohttp-lwt, lwt, base-unix, ounit ? null,
  findlib }:
...
assert stdenv.lib.versionAtLeast (stdenv.lib.getVersion jbuilder)
  "1.0+beta10";
assert stdenv.lib.versionAtLeast (stdenv.lib.getVersion conduit-lwt-unix)
  "1.0.3";
assert stdenv.lib.versionAtLeast (stdenv.lib.getVersion fmt) "0.8.2";
assert stdenv.lib.versionAtLeast (stdenv.lib.getVersion cohttp-lwt) "1.0.2";
assert stdenv.lib.versionAtLeast (stdenv.lib.getVersion lwt) "3.0.0" &&
  stdenv.lib.versionOlder (stdenv.lib.getVersion lwt) "4.0.0";
...
stdenv.mkDerivation rec {
  ...
  buildInputs = [
    ocaml jbuilder conduit-lwt-unix cmdliner magic-mime logs fmt cohttp-lwt
    lwt base-unix ]
  ++
  stdenv.lib.optional
  doCheck
